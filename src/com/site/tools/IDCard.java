...
public class IDCard {
	...
	public static String IDCardValidate(String IDStr) {
		...
		// ================ 出生年月是否有效 ================
		/*
		 * String strYear = Ai.substring(6, 10);// 年份 String strMonth =
		 * Ai.substring(10, 12);// 月份 String strDay = Ai.substring(12, 14);// 月份
		 * if (isDate(strYear + "-" + strMonth + "-" + strDay) == false) {
		 * errorInfo = "身份证生日无效。"; return errorInfo; } GregorianCalendar gc =
		 * new GregorianCalendar(); SimpleDateFormat s = new
		 * SimpleDateFormat("yyyy-MM-dd"); try { if ((gc.get(Calendar.YEAR) -
		 * Integer.parseInt(strYear)) > 150 || (gc.getTime().getTime() -
		 * s.parse( strYear + "-" + strMonth + "-" + strDay).getTime()) < 0) {
		 * errorInfo = "身份证生日不在有效范围。"; return errorInfo; } } catch
		 * (NumberFormatException e) { e.printStackTrace(); } catch
		 * (java.text.ParseException e) { e.printStackTrace(); } if
		 * (Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0)
		 * { errorInfo = "身份证月份无效"; return errorInfo; } if
		 * (Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0) {
		 * errorInfo = "身份证日期无效"; return errorInfo; }
		 */
		// =====================(end)=====================
